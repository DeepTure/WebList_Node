...
router.post("/comprobateCode", (req, res) => {
    const data = req.body;
    db.query(
        "SELECT idUser,time from token where idtoken=?",
        [data.code],
        (err, id) => {
            if (err) res.send(err);
            if (Array.isArray(id) && !(id.length === 0)) {
                ...
                if (
                    hoy.getDate() == caducacion.getDate() &&
                    hoy.getMonth() == caducacion.getMonth() &&
                    hoy.getFullYear() == caducacion.getFullYear()
                ) {
                    if (
                        hoy.getHours() >= caducacion.getHours() &&
                        hoy.getHours() <= caducacion.getHours() + 3
                    ) {
                        //Debe tener una caducación de tres horas
                        db.query(
                            "DELETE FROM token WHERE idtoken = ?",
                            [data.code],
                            (err, deleted) => {
                                if (err) res.json(err);
                                ...
                                res.render("changepassword", {
                                    msj:
                                        "Usuario Identificado, cambie contraseña",
                                    icon: "success",
                                    info: data.id,
                                    rol: data.rol,
                                });
                        ...
                        db.query(
                            ...
                            (err, deleted) => {
                                if (err) res.json(err);
                                res.render("comprobateCode", {
                                    correo:
                                        "Acceso denegado, su token acaba de caducar",
                                    icon: "error",
                                });
                            }
                        );
                    ...
                    db.query(
                        ...
                        (err, deleted) => {
                            if (err) res.json(err);
                            res.render("comprobateCode", {
                                correo: "Acceso denegado, su token caducó",
                                icon: "error",
                            });
                        }
                    );
                ...
                res.render("comprobateCode", {
                    correo: "Acceso denegado, su token no existe",
                    icon: "error",
                });
            }
        }
...
router.post("/comprobateEmail", (req, res) => {
    const data = req.body;
    db.query(
        "SELECT contraseña,numEmpleado FROM profesor WHERE correo=?",
        [data.correo],
        (err, numEmpleado) => {
            if (err) res.json(err);
            if (Array.isArray(numEmpleado) && !(numEmpleado.length === 0)) {
                ...
                res.render("comprobateCode", {
                    correo:
                        "Se le ha enviado un correo con un token para recuperar su contraseña",
                    icon: "success",
                    info: numEmpleado[0].numEmpleado,
                    rol: "profesor",
                });
            } else {
                db.query(
                    "SELECT contraseña,boleta FROM alumno WHERE correo=?",
                    [data.correo],
                    (err, boleta) => {
                        if (err) res.json(err);
                        if (Array.isArray(boleta) && !(boleta.length === 0)) {
                            ...
                            res.render("comprobateCode", {
                                correo:
                                    "Se le ha enviado un correo con un token para recuperar su contraseña",
                                icon: "success",
                                info: boleta[0].boleta,
                                rol: "alumno",
                            });
                        } else {
                            db.query(
                                "SELECT contraseña,idAdmin FROM administrador WHERE correo=?",
                                [data.correo],
                                (err, id) => {
                                    if (err) res.json(err);
                                    if (
